...
class OrderServiceTest {
    ...
    void createOrderReducesStock() {
        ...
        itemReq.setName("Book"); itemReq.setSku("BOOK-9"); itemReq.setPrice(10.0); itemReq.setStock(5);
        var item = itemService.create(itemReq);

        CustomerRequest custReq = new CustomerRequest();
        custReq.setFullName("Ada"); custReq.setEmail("ada2@example.com");
        var customer = customerService.create(custReq);

        OrderLineRequest line = new OrderLineRequest();
        line.setItemId(item.getId()); line.setQuantity(2);

...
class CustomerServiceTest {
    ...
    @BeforeEach
    void setUp() { service = new CustomerService(new CustomerRepository(), new CustomerMapper()); }

    private CustomerRequest req(String name, String email) {
        CustomerRequest r = new CustomerRequest();
        r.setFullName(name); r.setEmail(email); r.setCity("NYC");
        return r;
    }

    @Test
    void createCustomer() {
        assertNotNull(service.create(req("Ada", "ada@example.com")).getId());
